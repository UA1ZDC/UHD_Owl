...
//  - single clock: posedge sclk (plus optional async reset)
//  - fixed 32-bit frame; commit on end-of-frame pulse
//
// Patch highlights:
//  1) Removed reliance on `initial` for HW defaults by adding an async reset
//     driven from GPIO[3] (CPLD_RST_N) that you already reserved in UHD.
//     - We still KEEP the initial block for simulation convenience.
//     - For deterministic HW defaults, make FPGA pulse CPLD_RST_N low once
//       after FPGA config.
//  2) Added frame_end_seen to make "frame end" a ONE-CLOCK PULSE even if
//     master keeps toggling SCLK while CS stays low.
// ============================================================================
...
module regfile_compact #(
    ...
    // --------------------------------------------------------------------
    // External reset from FPGA via GPIO[3] (CPLD_RST_N).
    // Per your UHD GPIO map, bit3 is reserved for CPLD_RST_N.
    // gpio0_now bit0..7 == {io_00..io_07}, so gpio0_now[3] == cpld_io_03.
    // --------------------------------------------------------------------
    wire rst_n = gpio0_now[3];
    ...
    // Power-up defaults (simulation help). HW-deterministic defaults are via rst_n.
    initial begin
        ctrl0_r        = CTRL0_DFLT8;
        ctrl1_r        = CTRL1_DFLT8;
    ...
    wire frame_end_lv = frame_active && (bitcnt == 6'd31);

    // Make frame_end a one-clock pulse (in case extra clocks are present).
    reg frame_end_seen;
    wire frame_end_pulse = frame_end_lv && !frame_end_seen;
    ...
    // ---------------------------
    // Sequential logic (single clock domain + async reset)
    // ---------------------------
    always @(posedge sclk or negedge rst_n) begin
        if (!rst_n) begin
            // External reset from FPGA (CPLD_RST_N low)
            ctrl0_r        <= CTRL0_DFLT8;
            ctrl1_r        <= CTRL1_DFLT8;
            att2_code_r    <= ATT2_CODE_DFLT7;
            att2_last_ok_r <= ATT2_LAST_OK_DFLT;
            ...
            // New frame: clear pendings and re-arm frame_end_pulse
            ...
            if (frame_active) begin
                // If we reached the end once, remember it.
                if (frame_end_lv) begin
                    frame_end_seen <= 1'b1;
                end
                ...
                // Commit exactly once at the end of fixed 32-bit frame
