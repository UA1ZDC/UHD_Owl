...
//
// LE pulse:
//  - In the original version LE was generated as (le_armed && sclk). If the
//    SPI master violated the protocol and kept toggling SCLK after frame end
//    while CS stayed active, LE could pulse multiple times.
//  - In this patched version we generate a one-shot LE request (le_pulse_req)
//    that is asserted once at frame end and then auto-clears.
//
// NOTE: This pairs well with spi_engine_patched.v where evt_frame_end is also
// made a one-clock pulse (so W1 actions can't retrigger).
// ============================================================================
...
module att2_engine (
    ...
    input  wire mosi_raw,     // raw MOSI for true AUTOLATCH
    ...
    // One-shot LE request (asserted once, then auto-clears)
    reg le_pulse_req;
    ...
    // Busy = mode active or LE pulse pending
    assign att2_busy = (mode != 2'd0) || le_pulse_req;

    // LE output (one-shot request). Polarity controlled by CTRL0.
    // We intentionally do NOT gate LE with SCLK here: pulse width becomes
    // "from frame_end until next SCLK edge or CS deassert" which is robust.
    wire le_level = le_pulse_req;
    assign ATT2_RX_LE = ctrl0_le_pol ? ~le_level : le_level;

    // Power-up init for state regs (helps sim / avoids unknown start)
    initial begin
        mode         = 2'd0;
        le_pulse_req = 1'b0;
        att2_mosi_reg = 1'b0;
    ...
    always @(posedge sclk or posedge cs_n) begin
        if (cs_n) begin
            ...
            // Default: auto-clear LE request after one SCLK (if it was set)
            // (set at frame_end has priority below)
            if (le_pulse_req) begin
                le_pulse_req <= 1'b0;
            end
